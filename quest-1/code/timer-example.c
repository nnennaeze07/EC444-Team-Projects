...
#define TIMER_INTERVAL_SEC   (1.00000000) // sample test interval for the first timer
//#define TIMER_INTERVAL1_SEC   (1.00000000)   // sample test interval for the second timer
#define TEST_WITHOUT_RELOAD   0        // testing will be done without auto reload
#define TEST_WITH_RELOAD      1        // testing will be done with auto reload

char str[16]; //make global;
unsigned short display_int;
...
static void timer_evt_task(void *arg) {
    ...
    while (1) {
        ...
        sprintf(str,"%hu",display_int);
        printf("Time   :%hu\n",display_int);

        if(display_int == 0){
          printf("Action\n");
          display_int = 25;
          //add servo stuff here?
...
static void test_alpha_display() {
    ...
    while (1) {
      if(ret == ESP_OK) { //if button is pushed, the check if statement
        ...
        //define a string in str here
          //vTaskDelay(1000 / portTICK_PERIOD_MS); READ COMMENT UNDER THIS
          /*Okay so if you run this without changing anything youll probably
          see some red comments every 5 seconds, and this is because theres
          no delay in this function, so it doesnt give the other priorities time
          to run while its running. The code will still work though. The reason
          I didnt include the delay is because it makes the display blink/flash
          and I dont like it, you can uncomment and see for yourself what I mean*/
...
static void timer_evt_task(void *arg) {
    while (1) {
        ...
        print_timer_counter(evt.timer_counter_val);
    }
}
...
>>>>>>> 0bf236e84945ab46790735d7b42208d99297a630
